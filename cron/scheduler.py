...
def _cron_mirror_delivery_enabled(job: dict, cfg: Optional[dict] = None) -> bool:
    ...
    try:
        ...
        if "mirror_delivery" in cron_cfg:
            return bool(cron_cfg.get("mirror_delivery"))
        # Backward compatibility with the older local config key.
        return bool(cron_cfg.get("mirror_deliveries_to_session", False))
    except Exception:
        return False


def _cron_mirror_deliveries_enabled(cfg: Optional[dict] = None) -> bool:
    """Backward-compatible alias for older local tests/config naming."""
    return _cron_mirror_delivery_enabled({}, cfg)

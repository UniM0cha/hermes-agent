...
class DiscordAdapter(BasePlatformAdapter):
    ...
    async def edit_message(
        ...
        *,
        ...
        try:
            ...
            if metadata and metadata.get("thread_id"):
                target_chat_id = str(metadata["thread_id"])
    ...
    async def delete_message(
        ...
        """Delete a Discord message, preserving thread routing metadata.

        Progress/status cleanup often stores the parent channel as ``chat_id``
        while the temporary bubble was actually sent into a thread.  When
        ``metadata.thread_id`` is present, resolve that thread directly before
        fetching/deleting the message.
        """
        if not self._client:
            return False
        try:
            ...
            msg = await channel.fetch_message(int(message_id))
            await msg.delete()
            self._last_overflow_preview.pop((str(target_chat_id), str(message_id)), None)
            return True
        except Exception:
            logger.debug("[%s] Failed to delete Discord message %s", self.name, message_id, exc_info=True)
            return False

...
| **Free-response channels** | You can make specific channels mention-free with `DISCORD_FREE_RESPONSE_CHANNELS`, or disable mentions globally with `DISCORD_REQUIRE_MENTION=false`. These channels still auto-create threads when `DISCORD_AUTO_THREAD=true`; add the channel to `DISCORD_NO_THREAD_CHANNELS` if you want inline replies. |
...
| `DISCORD_AUTO_THREAD` | No | `true` | When `true`, automatically creates a new thread for every handled message in a text channel, including free-response channels, so each conversation is isolated (similar to Slack behavior). Messages already inside threads or DMs are unaffected. |
